...
def test_load_testing(client, db_connection):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(submit_request) for _ in range(25)] # 25 requests x 5 records = 125 total
        results = [f.result() for f in futures]
    
    assert all(r.status_code == 202 for r in results)
    
    # Verify flush occurred by checking database
    records_count = db_connection.cursor().execute("SELECT COUNT(*) FROM people").fetchone()[0]
