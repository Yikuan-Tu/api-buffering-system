...
sys.path.append(str(Path(__file__).parent.parent)) # Add parent directory to sys.path to reuse modules/pkgs in app/
...
def run_load_test(num_requests=20):
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(test_submit) for _ in range(num_requests)] # default to 20 requests x 5 records = 100 total
...
def verify_flush_occurred():
    ...
    if count >= BUFFER_SIZE:
        ...
        print(f"\n❌ VERIFICATION: Only {count} records in database (flush may have failed)")
        return False
